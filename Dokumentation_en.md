...
*   **RAG (Retrieval Augmented Generation):** An approach where relevant information is retrieved from a knowledge base before response generation. Supports `.txt`, `.md`, `.xlsx`, and `.docx` files. Used by M.Y.R.A. in direct chat and by both AIs in the Dual AI conversation.
...
*   **Upload File (.txt, .md, .xlsx, .docx):** Button and file selection for uploading text, Markdown, Excel, or Word files.
...
*   **Automatic Loading:** On application startup, `Dokumentation_en.md`/`_de.md` and all `.md` files from the `docs` directory (with the appropriate language suffix) are automatically loaded.
...
Contains the basic structure of the HTML page. Includes Tailwind CSS, Heroicons, and JavaScript modules. Defines an `importmap` for external libraries like React, ReactDOM, `@google/genai`, `uuid`, `chart.js`, `react-chartjs-2`, `react-markdown`, `remark-gfm`, `react-syntax-highlighter`, `read-excel-file`, and `mammoth`. Sets initial theme and language from `localStorage`.

### 4.3 `metadata.json`

Application metadata, name, description, requested permissions (e.g., microphone for voice input).
...
*   **Logic:**
    ...
    *   The right main panel is dedicated to the `ChatInterface` for direct interaction with the selected main AI (`myraConfig.activeChatAgent`).
...
*   **Core Functions:**
    *   **State Management:** Manages the state for M.Y.R.A. and C.A.E.L.U.M. in parallel.
    *   **Configuration Management (`myraConfig`, `updateMyraConfig`).**
    *   **Simulation Loops (`simulateNetworkStepMyra`, `simulateNetworkStepCaelum`).**
    *   **AI Interaction (`generateActiveAgentChatResponse`, `startDualConversation`).** The `generateActiveAgentChatResponse` function sends messages to the AI selected in `myraConfig.activeChatAgent`.
    *   **System Instructions (`getMyraBaseSystemInstruction`, `getCaelumBaseSystemInstruction`).**
    *   **Knowledge Base (RAG):**
        *   `loadDocumentationKnowledge`: Automatically loads documentation files on startup.
        *   `loadAndProcessFile`: Now also handles `.xlsx` and `.docx` files using `read-excel-file` and `mammoth` respectively.
...
For direct interaction with the active main AI (M.Y.R.A. or C.A.E.L.U.M., depending on `myraConfig.activeChatAgent`). Includes Speech-to-Text (STT) and Text-to-Speech (TTS) functionality.
...
*   **Functionality:** Loads and renders selected `.md` files based on the app's current language setting. Uses `react-markdown` and `react-syntax-highlighter`.

#### 4.10.5 `IconComponents.tsx`

Contains SVG icons, including `DocumentTextIcon` for the documentation tab and icons for STT/TTS.

#### 4.10.6 `KnowledgePanel.tsx`

Manages the knowledge base (RAG). The file input now accepts `.txt`, `.md`, `.xlsx`, `.docx`.
...
Encapsulates the logic for communication with AI APIs.
...
(Description remains the same, `generateActiveAgentChatResponse` now for the active AI)

### 5.3 Knowledge Processing (RAG & Automatic Document Loading)

*   **Automatic Loading:** On application startup, `loadDocumentationKnowledge` in `useMyraState` loads the content of documentation files.
*   **Manual Upload:** Via the `KnowledgePanel`, `.txt`, `.md`, `.xlsx`, and `.docx` files can now be uploaded. Their text content is extracted, split into chunks, and stored in IndexedDB.
...
*   [**Knowledge Base & RAG**](./docs/config_knowledge_rag_en.md) (now includes information on `read-excel-file` for `.xlsx`)
...
The application will request microphone permissions when voice input is first used.