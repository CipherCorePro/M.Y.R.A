...
// Placeholder icons for missing ones - ideally replace with actual icons if available
// These are used if not found in IconComponents.tsx
...
const getNodeIcon = (type: NodeState['type']) => {
  switch(type) {
    case 'semantic': return <Cog6ToothIcon className="w-4 h-4 text-blue-400"/>; // Corrected icon
...
const NodePanel: React.FC<NodePanelProps> = ({ nodeStates }) => {
  ...
  return (
    <div className="p-3 bg-gray-700/50 backdrop-blur-sm rounded-lg shadow-lg border border-gray-600 space-y-4">
      <div>
        <h3 className="text-lg font-semibold mb-2 text-purple-300">Modulator Nodes</h3>
        {modulatorNodes.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {modulatorNodes.map(node => <NodeCard key={node.id} node={node} />)}
          </div>
        ) : (<p className="text-sm text-gray-400 italic">No modulator nodes active.</p>)}
      </div>
      <div>
        <h3 className="text-lg font-semibold mb-2 text-purple-300">Semantic Nodes (Sample)</h3>
         {semanticNodes.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {semanticNodes.slice(0,4).map(node => <NodeCard key={node.id} node={node} />)}
          </div>
        ) : (<p className="text-sm text-gray-400 italic">No semantic nodes loaded.</p>)}
        {semanticNodes.length > 4 && <p className="text-xs text-gray-400 mt-2">Showing first 4 of {semanticNodes.length} semantic nodes...</p>}
      </div>
    </div>
  );
};
