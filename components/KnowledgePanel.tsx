...
const KnowledgePanel: React.FC<KnowledgePanelProps> = ({
  ...
  return (
    <div className="p-2 sm:p-3 bg-gray-700/50 backdrop-blur-sm rounded-lg shadow-lg border border-gray-600 space-y-4 sm:space-y-6 max-h-[calc(100vh-120px)] sm:max-h-[calc(100vh-150px)] overflow-y-auto fancy-scrollbar">
      <div className="flex items-center mb-1">
        <BookOpenIcon className="w-5 h-5 sm:w-6 sm:h-6 mr-2 text-purple-300" />
        <h3 className="text-lg sm:text-xl font-semibold text-purple-300">{t('knowledgePanel.title')}</h3>
      </div>

      <div className="space-y-3 p-2 sm:p-3 bg-gray-800/30 rounded-md border border-gray-600/50">
        ...
        <button
          ...
          className="w-full py-2 px-4 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 transition duration-150 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed text-sm"
        >
          {isLoading ? (
      ...
      <div className="space-y-3 p-2 sm:p-3 bg-gray-800/30 rounded-md border border-gray-600/50">
        <div className="flex flex-col sm:flex-row justify-between sm:items-center space-y-2 sm:space-y-0">
          ...
          {sources.length > 0 && (
             <button
                ...
                disabled={isLoading}
                className="w-full sm:w-auto py-1.5 sm:py-1 px-3 bg-red-600 text-white text-xs font-semibold rounded-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 transition disabled:opacity-50"
        ...
        {sources.length === 0 ? (
          ...
          <ul className="space-y-1 max-h-48 sm:max-h-60 overflow-y-auto fancy-scrollbar pr-1">
            {sources.map(source => (
              <li key={source.name} className="text-xs sm:text-sm text-gray-300 bg-gray-700/50 p-1.5 sm:p-2 rounded-md flex justify-between">
                <span className="truncate max-w-[70%]">{source.name}</span>
                <span className="text-xs text-gray-400 whitespace-nowrap">{t('knowledgePanel.sources.chunkCount', { count: String(source.chunkCount) })}</span>
              </li>
            ))}
          </ul>
        )}
